        ...
        public class Farmer_draw_Patch
        {
            public static void Prefix(Farmer __instance, SpriteBatch b)
            {
                ...
                if (__instance.IsLocalPlayer)
                {
                    ...
                    { // else (if moving right/down OR left/up) then calculates the difference between x and y speed and sees if that is less than half of the greater speed because if they are close in value, its probably diagonal but if one is greater than the other, its probably horizontal/vertical
                        source = 1; // NW/SE board direction
                    }
                    else if (((speed.X > 0 && speed.Y < 0) || (speed.X < 0 && speed.Y > 0)) & Math.Abs(Math.Abs(speed.X) - Math.Abs(speed.Y)) < Math.Max(Math.Abs(speed.X), Math.Abs(speed.Y)) / 2)
                    {// same as above but right/up or left/down
                        source = 0; // NE/SW board direction
                    }
                    else if (Math.Abs(speed.Y) > Math.Abs(speed.X))
                    {
                        source = 3; // N/S board direction
                    }
                    else
                    {
                        source = 2; // W/E board direction
                    }
                    __instance.modData[sourceKey] = source.ToString();
                }

                __instance.drawOffset = new Vector2(0, -24);
                ...
                b.Draw(boardTexture, Game1.GlobalToLocal(__instance.Position) - offset, new Rectangle(source * 16, 0, 16, 16), Color.White, 0, Vector2.Zero, 5, SpriteEffects.None, __instance.getDrawLayer() - 0.00001f); // draw board in whichever orientation makes sense for direction from previous calculations
            }
        }
        ...
        public class Object_drawWhenHeld_Patch
        {
            public static bool Prefix(Object __instance, SpriteBatch spriteBatch, Vector2 objectPosition, Farmer f)
            {
                if (!Config.ModEnabled || !__instance.modData.ContainsKey(boardKey))
                    return true;
                spriteBatch.Draw(boardTexture, objectPosition /*+ new Vector2(0, 92)*/, new Rectangle(32, 0, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, Math.Max(0f, (f.StandingPixel.Y + 3) / 10000f));
                return false;
            }
        }
